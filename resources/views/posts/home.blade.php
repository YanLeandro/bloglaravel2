...
@section('content')
    <section class="w-full bg-gray-200 py-4 flex-row justify-center text-center">
        <h2 class="py-4 text-3xl">Acerca de </h2>
    ...
    <section class="w-full">
        <div class="flex justify-center">
            <div class="max-w-6xl text-center">
                <h2 class="py-4 text-3xl border-solid border-gray-400 border-b-2">Ultimos posts</h2>
                <div class="flex flex-wrap justify-between">
                    @foreach($posts as $post)
                        <article style="width:300px" class="text-left p-2">
                            <h3 class="py-2 text-xl text-white bg-purple-400 rounded">{{$post->title}}</h3>
                            <p>{{$post->get_limit_body}} <a class="font-bold text-blue-700 no-underline hover:underline" href="{{ route('posts.detail', $post->slug) }}">Read more</a></p>
                        </article>
